...
def do_train(cfg, backbone,
    ...
    for iteration, (source_images, source_targets, _) in enumerate(source_data_loader, start_iter):
        p = float(iteration + iteration * 16) / (400 * 16)
        ...
        source_domain_output = domaindiscriminator(source_features[0], alpha)
        err_s_domain = domain_loss(source_domain_output, real=False)
        target_domain_output = domaindiscriminator(target_features[0], alpha)
        err_t_domain = domain_loss(target_domain_output, real=True)
        loss = detection_loss + cfg.SOLVER.LAMBDA * (err_s_domain + err_t_domain)

        meters.update(reg_loss=reg_loss)
        meters.update(cls_loss=cls_loss)
        meters.update(detection_loss=detection_loss)
        meters.update(err_s=err_s_domain)
        meters.update(err_t=err_t_domain)
        ...
        if iteration % args.log_step == 0:
            ...
            if summary_writer:
                ...
                summary_writer.add_scalar('losses/err_s_loss', err_s_domain, global_step=global_step)
                summary_writer.add_scalar('losses/err_t_loss', err_t_domain, global_step=global_step)
